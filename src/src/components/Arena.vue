...
const BuffIconUrl = {
  ...
  OnFire: "data:image/png;base64,",
...
class Freeze extends ComboSkill {
  ...
  Act(targets) {
    if (this.stage == 0) {
      ...
      target.DealDamage(o, dmg, SourceType.Ranged, false)
      Renderer.Comma()
      target.UpdateBuff(new Frozen(3000, target))
      Renderer.EndLine()
      this.owner.combo = null
      this.stage = 0
    }
...
var Renderer = {
  ...
  Run: async function(id) {
    ...
    let maxScroll = mainPage.scrollTop - mainPage.clientHeight
    let checkScroll = maxScroll - mainPage.scrollTop < 50 || (mainPage.scrollTop / maxScroll) > 0.95
    ...
    if (checkScroll) {
      mainPage.scrollTop = mainPage.scrollHeight - mainPage.clientHeight
    }
  },
...
var Render = {
  ...
  Name: function(name, _class = "") {
    let str = ''
    let fixedName = Render.EscapeHTML(name)
    str += `<span class="name ${_class}">${fixedName}</span>`
    return str
  },
  ...
  EscapeHTML: function(str) {
    return str.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/'/g, '&apos;').replace(/"/g, '&quot')
  }
}
